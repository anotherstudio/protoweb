...
(function (window, factory) {
    ...
    let loggers = {
        ...
        append: (response, path, items) =>
            `${items.length} items. URL: ${path}`,
    ...
    proto.updateGetPath = function () {
        ...
        if (!optPath) {
            console.error(
                `InfiniteScroll path option required. Set as: ${optPath}`
            );
            return;
        }
        // function
    ...
    proto.updateGetPathSelector = function (optPath) {
        ...
        if (!pathParts) {
            console.error(
                `InfiniteScroll unable to parse next link href: ${href}`
            );
            return;
        }
...
(function (window, factory) {
        ...
        let fetchPromise = fetch(path, fetchOptions)
            .then((response) => {
                ...
                return response[responseBody]().then((body) => {
                    let canDomParse =
                        responseBody == "text" && domParseResponse;
                    if (canDomParse) {
    ...
    InfiniteScroll.create.prefill = function () {
        ...
        if (!append) {
            console.error(
                `append option required for prefill. Set as :${append}`
            );
            return;
        }
